      ...
      <div className='min-h-[200px] flex flex-col lg:flex-row gap-4'>
        ...
        {loading ? (
          <div className='grid gap-1 w-full'>
            ...
            <h2 className='text-2xl lg:text-4xl font-medium h-6 lg:h-8  bg-slate-200 animate-pulse w-full'> 'Default Product Name'</h2>
          ...
          (
            <div className='flex flex-col gap-1'>
              <p className='bg-red-200 text-red-600 px-2 rounded-full inline-block w-fit'>{data?.brandName || "Brand Name"}</p> {/* Default text for brand name */}
              <h2 className='text-2xl lg:text-4xl font-medium'>{data?.productName || "Product Name"}</h2> {/* Default text for product name */}
              <p className='capitalize text-slate-400'>{data?.category || "Category"}</p> {/* Default text for category */}
              ...
              <div className='flex items-center gap-2 text-2xl lg:text-3xl font-medium my-1'>
                <p className='text-red-600'>{displayINRCurrency(data.sellingPrice) || "₹0"}</p> {/* Default price */}
                <p className='text-slate-400 line-through'>{displayINRCurrency(data.price) || "₹0"}</p> {/* Default price */}
              </div>
              ...
              <div>
                ...
                <p>{data?.description || "No description available."}</p> {/* Default description */}
              </div>
            </div>
          )
        }
...

      {
        data.category && (
          <>
            ...
            <VerticalCardProduct category={data.category} heading={`More ${data.category}`} /> {/* Added VerticalCardProduct */}
          </>
        )
      }
    </div>
