...
const HorizontalCardProduct = ({ category, heading = "Default Heading" }) => { // Add a default heading
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const loadingList = new Array(13).fill(null);

  const scrollElement = useRef(); // Keeping this since we use it for scrolling
  ...
  return (
    <div className='container mx-auto px-4 my-6 relative'>
      <h2 className='text-2xl font-semibold py-4'>
        {heading || "Products"} {/* Ensure heading always has content */}
      </h2>
            ...
            <div key={index} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
              ...
              <div className='p-4 grid w-full gap-2'>
                <h2 className='font-medium text-base md:text-lg text-ellipsis line-clamp-1 text-black bg-slate-200 animate-pulse p-1 rounded-full'>
                  Loading...
                </h2> {/* Provide temporary content during loading */}
            ...
            <Link to={"product/" + product?._id} key={product._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
              ...
              <div className='p-4 grid'>
                <h2 className='font-medium text-base md:text-lg text-ellipsis line-clamp-1 text-black'>
                  {product?.productName || "Product Name"} {/* Fallback to default content */}
                </h2>
